...
connect_fn = async function() {
  try {
    await import_mongoose.default.connect(__privateGet(_Mongoose, _uri), __privateGet(_Mongoose, _options));
  } catch (err) {
  ...
  static IndexManager(schema, options = { indexes: [] }) {
    return function(schema2) {
      ...
      schema2.post("save", function(doc) {
        const db2 = import_mongoose2.default.connection.db;
        if (!db2) {
          console.warn("MongoDB connection not established yet.");
          return;
        }
  ...
  static AutoPopulate(schema, options = { paths: [] }) {
    return function(schema2) {
      ...
      schema2.pre("save", function(next) {
        paths.forEach((path3) => {
          if (this[path3] && import_mongoose3.default.isObjectIdOrHexString(this[path3])) {
            this.populate(path3);
          }
        });
        next();
    ...
    return function(schema2) {
      ...
      schema2.pre("save", function(next) {
        Object.keys(fields).forEach((field) => {
          if (this[field] && import_mongoose3.default.isObjectIdOrHexString(this[field])) {
...
var JWTUtil = class {
  static async generateTokens({
    ...
    if (generateRefreshToken) {
      return {
        accessToken: import_jsonwebtoken2.default.sign(tokenPayload.payload || {}, secret, signOptions),
      ...
      return import_jsonwebtoken2.default.sign(payload || {}, secret, signOptions);
    }
  }
  static async generateRefreshToken({
    ...
    return import_jsonwebtoken2.default.sign(payload, REFRESH_SECRET, signOptions);
  }
  ...
  static async verifyRefreshToken({
    ...
    try {
      ...
      const payload = import_jsonwebtoken2.default.verify(token, REFRESH_SECRET);
      return payload;
    } catch {
