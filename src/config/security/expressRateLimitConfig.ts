...
const config = {
  ...
  ): Partial<Parameters<typeof rateLimit>[0]> => {
    return {
      ...
      identifier: config?.identifier || undefined, // Optional custom identifier for the policy
      store: config?.store || undefined, // Uses in-memory store by default
      passOnStoreError: config?.passOnStoreError || false, // Do not pass if store fails (defaults to false)
      keyGenerator: config?.keyGenerator || ((req) => req.ip as string), // Identify users by their IP address
